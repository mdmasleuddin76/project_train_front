...
const Stock = () => {
  ...
  const portfolioSummary = useMemo(() => {
    const totalMarketValue = stockHoldings.reduce(
      (sum, s) => sum + (s.currentPrice * s.stockQuantity || 0),
      0
    );
    const totalInvestment = stockHoldings.reduce(
      (sum, s) => sum + s.stockPrice * s.stockQuantity,
      0
    );
    const totalGainLoss = stockHoldings.reduce(
      (sum, s) => sum + (s.gain || 0),
      0
    );
  ...
  if (isLoading)
    return (
      <div className="flex justify-center items-center h-screen bg-gray-900 text-lg text-gray-400">
          ...
          <div className="bg-gray-800 p-6 rounded-xl mb-8 shadow-lg border border-gray-700">
            ...
            >
              <select
                ...
                className="bg-gray-700 border-gray-600 rounded-lg p-3 focus:ring-indigo-500 focus:border-indigo-500 w-full"
                required
              >
              ...
              <input
                type="number"
                step="0.01"
                placeholder="Purchase Price / Share"
              ...
              <button
                type="submit"
                className="bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-5 rounded-lg w-full"
              >
                Save Stock
              </button>
            </form>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-gray-800 p-6 rounded-xl shadow-lg border-gray-700 hover:border-indigo-500 transition-all">
            ...
            <p className="text-3xl font-bold mt-2 text-white">
              ₹
              {portfolioSummary.totalMarketValue.toLocaleString("en-IN", {
            ...
            <thead className="bg-gray-700/50">
              <tr>
                {[
                  ...
                  <th
                    key={header}
                    className="p-4 text-sm font-semibold text-gray-300 uppercase tracking-wider"
                  >
                    {header}
                  </th>
                ))}
                ...
                stockHoldings.map((s) => (
                  ...
                  >
                    ...
                    <td className="p-4 whitespace-nowrap font-bold">
                      {s.currentPrice ? (
                        `₹${(s.currentPrice * s.stockQuantity).toLocaleString(
                          "en-IN"
                        )}`
                      ) : (
                        <span className="text-gray-500">N/A</span>
                    ...
                    <td
                      className={`p-4 whitespace-nowrap font-semibold ${
                        s.gain >= 0 ? "text-green-400" : "text-red-400"
                      }`}
                    >
                      {s.gain ? (
                        `${s.gain >= 0 ? "+" : ""}₹${s.gain.toLocaleString(
                          "en-IN"
                        )}`
                      ) : (
                        <span className="text-gray-500">N/A</span>
                    ...
                    <td
                      className={`p-4 whitespace-nowrap font-semibold ${
                        s.gainPercent >= 0 ? "text-green-400" : "text-red-400"
                      }`}
                    >
                      {s.gainPercent ? (
